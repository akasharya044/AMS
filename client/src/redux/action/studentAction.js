...
const fetchDatewiseAttendenceHelper = (data) => {
    return {
        type: "GET_DATEWISE",
        payload: data
    }
...
export const fetchDatewiseAttendance = () => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Get',
                url: url + "/api/student/datewiseAttendance"
            })
            dispatch(fetchDatewiseAttendenceHelper(data.result))
        }
        catch (err) {
            console.log("Error in sending message", err.message)
        }
