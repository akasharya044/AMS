...
export const markDatewise = (selectedStudents, subjectCode, department, year,
    section, attdate) => {
    return async (dispatch) => {
        try {
            await axios({
                method: 'Post',
                url: url + "/api/faculty/markDatewise",
                data: { selectedStudents, subjectCode, department, year, section, attdate }
            })
            ...
            console.log("Error in marking datewise attendence, faculty action", err.message)
        }
    }
}
